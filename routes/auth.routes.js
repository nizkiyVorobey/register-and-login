    ...
    (req, res) => {
        ...
        form.parse(req,
            async(err, fields, files) => {
                const customReq = { // express-validator can't work with custom fields, we should use template
                    body: {
                        ...fields
                    }
                };

                await check('email', 'Invalid email').isEmail().run(customReq);
                await check('userName', 'min length userName id 5 character').isLength({ min: 5 }).run(customReq);
                await check('password', 'min length userName id 7 character').isLength({ min: 7 }).run(customReq);

                const result = validationResult(customReq);
                ...
                const newPath = "/home/senya/work/register-login/uploadImages/" + Date.now() + files.avatar.name
